...
### 第零步：獲取 Google OAuth Client ID（如果使用 Google 登入）

如果你需要使用 Google 登入功能，需要先在 Google Cloud Console 創建 OAuth 客戶端 ID。

#### 步驟 1：創建 Google Cloud 項目（如果還沒有）
...
#### 步驟 3：創建 OAuth 2.0 客戶端 ID

1. **進入憑證頁面**（兩種方法）
   
   **方法 1：使用搜索欄（最快）**
   - 在頁面頂部的搜索欄中輸入「憑證」或「Credentials」
   - 點擊搜索結果中的「憑證」選項
   
   **方法 2：使用左側導航欄**
   - 點擊左側的「☰」選單圖標（如果導航欄已收起）
   - 展開「API 和服務」（API & Services）
   - 點擊「憑證」（Credentials）

2. **創建新的 OAuth 客戶端 ID**
   - 在憑證頁面，點擊頂部的 "+ 創建憑證"（+ CREATE CREDENTIALS）
   - 選擇 "OAuth 客戶端 ID"（OAuth client ID）

2. **配置 OAuth 同意畫面**（首次使用需要）
   - 如果提示配置 OAuth 同意畫面，選擇 "外部"（External）
   - 填寫應用程式名稱（例如：`Fruit Order System`）
   - 填寫用戶支持電子郵件
   - 點擊 "保存並繼續"
   - 在 "範圍" 頁面直接點擊 "保存並繼續"
   - 在 "測試用戶" 頁面可以添加測試用戶（可選）
   - 點擊 "返回儀表板"

3. **創建 OAuth 客戶端 ID**
   - 應用程式類型：選擇 "網頁應用程式"（Web application）
   - 名稱：輸入一個名稱（例如：`Fruit Order System Web Client`）

4. **配置已授權的 JavaScript 來源**
   - 點擊 "+ 新增 URI"
   - 添加以下 URL（根據你的實際部署情況）：
     ```
     http://localhost:3000
     ...
     ```
   - ⚠️ **重要**：必須包含你實際使用的所有域名（本地開發和生產環境）

5. **配置已授權的重新導向 URI**（如果需要）
   - 對於使用 `@react-oauth/google` 庫，通常不需要設置重新導向 URI
   - 如果需要，可以添加：
     ```
     http://localhost:3000
     https://fruit-order-system-1.onrender.com
...
6. **創建並複製 Client ID**
   - 點擊 "創建"
   - 會彈出一個對話框，顯示你的 **Client ID** 和 **Client Secret**
   - ⚠️ **重要**：複製 **Client ID**（格式類似：`123456789-abcdefghijklmnop.apps.googleusercontent.com`）
   - 這個 Client ID 就是你需要設置在環境變數中的 `your-google-client-id`
   - 可以點擊 "確定" 關閉對話框

#### 步驟 4：查看或管理你的 Client ID

如果之後需要查看或修改 Client ID：

1. **進入憑證頁面**（使用搜索欄輸入「憑證」或從左側導航欄進入「API 和服務」→「憑證」）
2. 在 "OAuth 2.0 客戶端 ID"（OAuth 2.0 Client IDs）部分找到你創建的客戶端
3. 點擊客戶端名稱可以查看詳情和編輯配置
4. 在詳情頁面的頂部可以看到完整的 **Client ID**（格式類似：`123456789-abcdefghijklmnop.apps.googleusercontent.com`）
5. 複製這個 Client ID，這就是你需要設置在環境變數中的值

#### 注意事項

- **Client ID vs Client Secret**：
  - 只需要 **Client ID**（用於前端和後端環境變數）
  - **Client Secret** 不需要（除非你使用服務端流程）
  
- **安全性**：
  - Client ID 可以公開（前端代碼中會使用）
  - 但建議不要將 Client Secret 暴露在前端代碼中

- **多個環境**：
  - 可以為開發環境和生產環境創建不同的 OAuth 客戶端
  - 或者在同一個客戶端中添加多個已授權的 JavaScript 來源
...
4. **設置環境變量**
   ...
   | `REACT_APP_GOOGLE_CLIENT_ID` | `your-google-client-id` | ⚠️ **Google OAuth 前端 Client ID**（必須與 `GOOGLE_CLIENT_ID` 相同） |
   | `GOOGLE_CLIENT_ID` | `your-google-client-id` | ⚠️ **Google OAuth 後端 Client ID**（必須與 `REACT_APP_GOOGLE_CLIENT_ID` 相同） |

   ⚠️ **重要提示**：
   - `SERVE_FRONTEND=true` **必須設置**，否則後端不會服務前端文件
   - `REACT_APP_API_URL` 在合併部署時可以留空，因為前後端在同一域名下
   - 如果設置 `REACT_APP_API_URL`，可以使用相對路徑 `/` 或完整 URL
   - **Google OAuth 配置**：如果使用 Google 登入，必須同時設置 `REACT_APP_GOOGLE_CLIENT_ID` 和 `GOOGLE_CLIENT_ID`，且**兩者的值必須完全相同**。這兩個環境變數都應該設置為同一個 Google OAuth Client ID。
...
- `REACT_APP_GOOGLE_CLIENT_ID` - 如果使用 Google 登入（前端使用）
- `GOOGLE_CLIENT_ID` - 如果使用 Google 登入（後端使用，**必須與 `REACT_APP_GOOGLE_CLIENT_ID` 相同**）
...
### 問題 7：Google OAuth 登入失敗 - invalid_client 錯誤

**症狀**：點擊 Google 登入按鈕後出現 "錯誤 401 : invalid_client" 或 "OAuth client was not found"

**解決方案**：
1. **確認環境變數設置**：
   - 在 Render 環境變數中，必須同時設置 `REACT_APP_GOOGLE_CLIENT_ID` 和 `GOOGLE_CLIENT_ID`
   - **兩個環境變數的值必須完全相同**（都應該是同一個 Google OAuth Client ID）
   - 檢查 Render 服務的環境變數設置，確認兩個變數都已設置且值相同

2. **檢查 Google Cloud Console 配置**：
   - 登入 [Google Cloud Console](https://console.cloud.google.com/)
   - 進入 "API 和服務" → "憑證"
   - 找到你的 OAuth 2.0 客戶端 ID
   - 確認 "已授權的 JavaScript 來源" 包含你的 Render 域名（如 `https://fruit-order-system-1.onrender.com`）
   - 確認 "已授權的重新導向 URI" 包含必要的重定向 URI

3. **驗證 Client ID 格式**：
   - Google OAuth Client ID 格式類似：`123456789-abcdefghijklmnop.apps.googleusercontent.com`
   - 確認沒有多餘的空格或換行符
   - 確認複製的是完整的 Client ID（不是 Client Secret）

4. **重新部署**：
   - 修改環境變數後，必須重新部署服務才能生效
   - 在 Render 服務頁面點擊 "Manual Deploy" 或等待自動部署

5. **檢查後端日誌**：
   - 在 Render 服務的 "Logs" 頁面查看後端日誌
   - 查找 "Checking Google Client ID" 相關的日誌訊息
   - 確認後端是否正確讀取到 `GOOGLE_CLIENT_ID` 環境變數
