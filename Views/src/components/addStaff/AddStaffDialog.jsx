  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${Getapi.api}/staff/insert`, {
        ...
        body: JSON.stringify({
          staffArray: staffMembers.map(staff => ({
            ...staff,
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        {staffMembers.map((staff, index) => (
          <Card key={index} sx={{ mb: 3, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
            <CardContent>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth error={!!errors[`${index}-job`]}>
                    <InputLabel id={`staff-job-label-${index}`}>Job Position</InputLabel>
                    <Select
                ...
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth error={!!errors[`${index}-locationId`]}>
                    <InputLabel id={`staff-location-label-${index}`}>
